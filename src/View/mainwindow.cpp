...
void MainWindow::get_sum_of_month() {
    ...
    ui->textBrowser->hide();
      if (ui->radioButton_an->isChecked()) {
        try {
            controller->annuityPaymentCalculation(ui->lineEdit_count_month->text().toLocal8Bit().data(),
                                                  ui->lineEdit_sum->text().replace(',','.').toLocal8Bit().data(),
                                                  ui->lineEdit_ratio->text().replace(',','.').toLocal8Bit().data()
                                                    );
            ui->result->setText("Ежемесячный платеж - " + QString::number(controller->get_data_annuityRatio(),'f',7) +
 " P\n" + "Переплата по кредиту - " + QString::number(controller->get_data_overpaymentLoan(),'f',7)
+ " P\n" + "Общая выплата - " + QString::number(controller->get_data_totalPayout(),'f',7) +" P" );
        } catch (...) {
     ...
     ui->textBrowser->show();
        try {
            controller->differentiatedPayment(ui->lineEdit_count_month->text().toLocal8Bit().data(),
                                              ui->lineEdit_sum->text().replace(',','.').toLocal8Bit().data(),
                                               ui->lineEdit_ratio->text().replace(',','.').toLocal8Bit().data());
        ui->result->setText("Переплата по кредиту - " + QString::number(controller->get_data_overpaymentLoan(),'f',7) +
" P\n" + "Общая выплата - " + QString::number(controller->get_data_totalPayout(),'f',7) +" P" );
    ui->textBrowser->append("№ Платеж по процентам\n");
        const double * array = controller->get_data_arrayOfMonthlyPayment_differentiatedPayment();
        for (int i = 0; i < int(controller->get_data_month()) ; i++)
        ui->textBrowser->append(QString::number(i + 1) + " - " + QString::number(array[i]) + "\n");
        } catch (...) {
        QMessageBox::critical(this,"Предупреждение","Что-то пошло не так...");
        }
}
...
void MainWindow::on_pushButton_graph_clicked()
{
    try {
      if (ui->lineEdit_X_max->isModified() && ui->lineEdit_X_min->isModified() &&
        ui->lineEdit_Y_max->isModified() && ui->lineEdit_Y_min->isModified() && !ui->result_show->text().isEmpty()) {
        controller->set_Xmax(ui->lineEdit_X_max->text().replace(',','.').toDouble());
        controller->set_Xmin(ui->lineEdit_X_min->text().replace(',','.').toDouble());
        controller->set_Ymax( ui->lineEdit_Y_max->text().replace(',','.').toDouble());
        controller->set_Ymin( ui->lineEdit_Y_min->text().replace(',','.').toDouble());
        QString expressionString = ui->result_show->text();
