...
void MainWindow::buttom_equal() {
    ...
    ui->result_show->setText(final_array);
    QByteArray str_bit = final_array.toLocal8Bit();
    char * str = str_bit.data();
    if (!is_valid_expression(str)) {
//        char * str_new = new_str(str);
//        stack * Node = NULL;
        char * str_x = NULL;
        QString double_x = MainWindow::edit_textLine();
        QByteArray str_bit_x = double_x.toLocal8Bit();
        str_x = str_bit_x.data();
//        RMN(&Node,str_new,str_x);
//        double result = calculating_expression(&Node);
        double result = calculate(str,str_x);
        final_array = QString::number(result,'g',15);
        ui->result_show->setText(final_array);
    } else {
